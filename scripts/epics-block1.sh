#!/usr/bin/env bash
set -euo pipefail
# Block 1 — console table
# Set your repo first (change if needed)
REPO=robnreb1/cerply

gh issue list -R "$REPO" --state all --label Epic,epic \
...
| (["#", "Epic", "Priority", "Status", "Updated"] | @tsv),
  (.[] | [ .num, .title, .prio, .status, .updated ] | @tsv)
'
