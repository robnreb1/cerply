...
export async function registerCertifiedArtifactsRoutes(app: FastifyInstance) {
  ...
  app.get('/api/certified/artifacts/:artifactId', { config: { public: true } }, async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    const result = await withPrisma(async (prisma) => {
      ...
      if (!artifact) {
        return { success: false, error: 'FILE_NOT_FOUND', message: 'artifact file not found on disk' };
      }

      // Add sha256 field to artifact for API response
      const artifactWithSha256 = { ...artifact, sha256: record.sha256 };
      return { success: true, data: artifactWithSha256, sha256: record.sha256 };
    ...
    reply.header('etag', `W/"${result.sha256}"`);
  ...
  app.get('/api/certified/artifacts/:artifactId.sig', { config: { public: true } }, async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    const result = await withPrisma(async (prisma) => {
      ...
      const signature = fromBase64(record.signature);
