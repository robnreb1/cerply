/* /api/ledger/totals → sum by model and grand total (DB-aware) */
export async function registerLedgerRoutes(app: any) {
  app.get('/api/ledger/totals', async (req: any, reply: any) => {
    ...
    // lazy-load model at call time
    const { genLedger } = require('../db/observability'); // kept for symmetry; not directly used here
