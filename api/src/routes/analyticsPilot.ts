...
module.exports.registerAnalyticsPilot = async function registerAnalyticsPilot(app: any) {
  app.get('/api/analytics/pilot', async (_req: any, reply: any) => {
    ...
    if (!db?.execute) return { ok: true, db: false, windowHours: 24, events: {}, ledger: { totalCents: 0, byModel: [] } };

    const since = new Date(Date.now() - 24 * 3600 * 1000);
    const eventsByType = await db.execute(
      `select type, count(*)::int as count from events where ts >= $1 group by type order by type`,
      [since]
    );

    const byModel = await db.execute(
      `select model_used as model, coalesce(sum(cost_cents),0)::int as cents
       from gen_ledger where created_at >= $1 group by model_used order by model_used`,
      [since]
    );
    ...
    // fold events array into object { type: count }
    const ev = Object.fromEntries((eventsByType||[]).map((r:any)=> [r.type, r.count]));
